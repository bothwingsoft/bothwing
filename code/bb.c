...
BB* lookup_bb(obj_node* obj, PtrdiffT offset)
{
    ...
    CLG_DEBUG(5, "  lookup_bb (Obj %s, off %#"PRwx"): %p\n",
	     obj->name, offset, bb);
    return bb;
}
...
obj_node* obj_of_address(Addr addr)
{
  ...
  if (obj->offset != offset) {
      ...
      CLG_DEBUG(0, "Mapping changed for '%s': %#"PRwx" -> %#"PRwx"\n",
  ...
  if (*seen_before) {
    if (bb->instr_count != n_instrs) {
      VG_(message)(Vg_DebugMsg, 
		   "ERROR: BB Retranslation Mismatch at BB %#"PRwx"\n", addr);
      VG_(message)(Vg_DebugMsg,
		   "  new: Obj %s, Off %#"PRwx", BBOff %#"PRwx", Instrs %u\n",
		   obj->name, obj->offset,
		   addr - obj->offset, n_instrs);
      VG_(message)(Vg_DebugMsg,
		   "  old: Obj %s, Off %#"PRwx", BBOff %#"PRwx", Instrs %u\n",
    ...
    CLG_DEBUG(5, "- get_bb(BB %#"PRwx"): seen before.\n", addr);
    return bb;
  }
...
void CLG_(delete_bb)(Addr addr)
{
    ...
    if (bb == NULL) {
	CLG_DEBUG(3, "  delete_bb (Obj %s, off %#"PRwx"): NOT FOUND\n",
		  obj->name, offset);
    ...
    CLG_DEBUG(3, "  delete_bb (Obj %s, off %#"PRwx"): %p, BBCC head: %p\n",

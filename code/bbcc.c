...
void CLG_(zero_bbcc)(BBCC* bbcc)
{
  ...
  CLG_DEBUG(1, "  zero_bbcc: BB %#"PRwx", Cxt %d "
	   "(fn '%s', rec %d)\n", 
	   bb_addr(bbcc->bb),
...
BBCC* lookup_bbcc(BB* bb, Context* cxt)
{
   ...
   CLG_DEBUG(2,"  lookup_bbcc(BB %#"PRwx", Cxt %d, fn '%s'): %p (tid %d)\n",
...
BBCC* new_bbcc(BB* bb)
{
   ...
   CLG_DEBUG(3, "  new_bbcc(BB %#"PRwx"): %p (now %d)\n",
...
void insert_bbcc_into_hash(BBCC* bbcc)
{
    ...
    CLG_DEBUG(3,"+ insert_bbcc_into_hash(BB %#"PRwx", fn '%s')\n",
...
static BBCC* clone_bbcc(BBCC* orig, Context* cxt, Int rec_index)
{
    ...
    CLG_DEBUG(3,"+ clone_bbcc(BB %#"PRwx", rec %d, fn %s)\n",
    ...
    CLG_DEBUG(2,"- clone_BBCC(%p, %d) for BB %#"PRwx"\n"
		"   orig %s\n"
		"   new  %s\n",
...
BBCC* CLG_(get_bbcc)(BB* bb)
{
   ...
   CLG_DEBUG(3, "- get_bbcc(BB %#"PRwx"): BBCC %p\n",
		bb_addr(bb), bbcc);
...
void CLG_(setup_bbcc)(BB* bb)
{
  ...
  CLG_DEBUG(3,"+ setup_bbcc(BB %#"PRwx")\n", bb_addr(bb));
  ...
  CLG_DEBUGIF(1) {
    ...
    VG_(printf)(" %08"PRwx" -> %08"PRwx", SP %08"PRwx"\n",
  ...
  CLG_DEBUG(3,"- setup_bbcc (BB %#"PRwx"): Cost %p (Len %d), Instrs %d (Len %d)\n",
